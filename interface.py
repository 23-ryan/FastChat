...
if __name__ == '__main__':
    ...
    while True:
        if (choice == '1'):
            MY_USERNAME, client_socket, client_pending_socket = handleSignIn(
                proxy, IP, PORT)
            # print(client_socket)
            break
        elif (choice == '2'):
            MY_USERNAME, client_socket, client_pending_socket = handleSignUp(
                proxy, IP, PORT)
            # print(client_socket)
            break
        elif (choice == '3'):
                ...
                if (choice == '1'):
                    ...
                    if (chatWith in DM):
                        ...
                        handleDM(MY_USERNAME, chatWith,
                                 client_socket, proxy, False)
                    elif (chatWith in group):
                        print("----->")
                        handleDM(MY_USERNAME, chatWith,
                                 client_socket, proxy, True)
